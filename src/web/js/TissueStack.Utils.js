TissueStack.Utils = {
	...
	transformWorldCoordinatesToPixelCoordinates : function(worldVector, transformationMatrix) {
		...
		for (var x=1;x<=datasets;x++) {
			...
			$("#dataset_" + x + "_toolbox_canvas").css({"width" : TissueStack.canvasDimensions.width * 0.8, "height" : 70});
			$("#dataset_" + x + "_contrast_box").css({"width" : TissueStack.canvasDimensions.width * 0.8, "height" : 70});
			$("#dataset_" + x + "_toolbox_canvas").attr("width", TissueStack.canvasDimensions.width * 0.8);
			$("#dataset_" + x + "_toolbox_canvas").attr("height", 70);
			$("#dataset_" + x + "_contrast_box").attr({"width" : TissueStack.canvasDimensions.width * 0.8});
			$("#dataset_" + x + "_contrast_box").attr({"height" : 70});
