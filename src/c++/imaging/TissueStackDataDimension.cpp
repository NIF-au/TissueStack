...
tissuestack::imaging::TissueStackDataDimension::TissueStackDataDimension(
		...
		_id(0), _name(name), _offset(offset), _numberOfSlices(number_of_slices),_sliceSize(slice_size), _width(0), _height(0), _resolutionMm(0)
{
...
tissuestack::imaging::TissueStackDataDimension::TissueStackDataDimension(
		...
		const std::string & name,
		const unsigned long long int number_of_slices) : _id(id), _name(name), _offset(0), _numberOfSlices(number_of_slices), _sliceSize(0), _width(0), _height(0), _resolutionMm(0)
{
	if (name.empty() || number_of_slices <=0)
...
const float tissuestack::imaging::TissueStackDataDimension::getResolutionMm() const
{
	return this->_resolutionMm;
}

void tissuestack::imaging::TissueStackDataDimension::setResolutionMm(const float resolution_mm)
{
