...
void            *get_all_slices_of_all_dimensions(void *args)/*t_vol *volume)*/
{
  t_vol		*volume;
  t_arguments	*a;
  int           i;
  ...
  while (i < volume->dim_nb)  
    {
      ...
      if (a->opt->dim_start_end[i][0] != -1 &&
	  a->opt->dim_start_end[i][1] != -1)
	{
...
void            get_all_slices_of_one_dimension(t_vol *volume, unsigned long *start, int current_dimension,
						long unsigned int *count, t_arguments *a)
{
  ...
  current_slice = (unsigned int)a->opt->dim_start_end[current_dimension][0];
  // set the last slice extracted
  max = (unsigned int)(a->opt->dim_start_end[current_dimension][1] == 0 ?
		       volume->size[current_dimension] : a->opt->dim_start_end[current_dimension][1]);
