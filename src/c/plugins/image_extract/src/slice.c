...
int get_nb_blocks_percent(t_image_extract *a, t_vol *volume) {
	...
	while (i < volume->dim_nb) {
		if (a->dim_start_end[i][0] != -1 && a->dim_start_end[i][1] != -1) {
...
void *get_all_slices_of_all_dimensions(void *args) {
	...
	while (i < volume->dim_nb && exit == 0) {
		...
		i++;
		if (a->info->percent == 1)
			exit = a->general_info->is_percent_paused_cancel(
					a->info->id_percent, a->general_info);
	}
...
unsigned char * get_raw_data(t_memory_mapping * memory_mappings, t_vol *volume,
...
void get_all_slices_of_one_dimension(t_vol *volume, unsigned long *start,
		int current_dimension, long unsigned int *count, t_image_args *a) {
	...
	if (a->general_info->tile_requests->is_expired(
	...
	max = (unsigned int) (
			a->dim_start_end[current_dimension][1] == 0 ?
					volume->size[current_dimension] :
					a->dim_start_end[current_dimension][1]);
	// init height and width compare to the dimension
	get_width_height(&height, &width, current_dimension, volume->dim_nb,
			volume->dim_name_char, volume->size);
	...
	while (current_slice < max && exit == 0) {
		...
		if (volume->raw_data != 1) {
			...
			data = malloc(volume->slices_max * sizeof(*data));
			memset(data, 0, (volume->slices_max * sizeof(*data)));
			pthread_mutex_lock(&(a->p->lock));
			miget_real_value_hyperslab(volume->minc_volume, MI_TYPE_UBYTE,
					start, count, data);
			pthread_mutex_unlock(&(a->p->lock));
		} else
			data = get_raw_data(a->general_info->memory_mappings, volume,
					current_dimension, current_slice, &free_data);
		// print image
		if (a->info->h_position == -1 && a->info->w_position == -1) {
			...
			while (a->info->start_h <= h_max_iteration && exit == 0) {
				...
				while (a->info->start_w <= w_max_iteration && exit == 0) {
					...
					print_image(data, volume, current_dimension, current_slice,
							width, height, a);
