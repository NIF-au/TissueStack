...
void alloc_and_init_colormap_space_from_lookup(float **new_colormap,
		float **source) {
	int i = 0;
...
void alloc_and_init_colormap_space_from_src(float **new_colormap,
		float **source) {
	...
	while (1) {
		...
		while (lines != NULL && lines[c_row_index] != NULL) {
			...
			while (values[c_column_index] != NULL) {
				colormap_tmp[number_of_lines + c_row_index][c_column_index] =
						atof(values[c_column_index]);
				c_column_index++;
			}
	...
	while (len > 0) {
		...
		if ((len = read(fd, buff, 8096)) > 0) {
			...
			while (lines[c_row_index] != NULL) {
				...
				c_column_index = 0;
				while (values[c_column_index] != NULL) {
					colormap_tmp[c_row_index][c_column_index] = atof(
							values[c_column_index]);
					c_column_index++;
				}
				free_null_terminated_char_2D_array(values);
				c_row_index++;
			...
			if (!stat(buf, &statbuf)) {
				if (!S_ISDIR(statbuf.st_mode)) {
					...
					if (discrete) {
						...
						int j = 0;
						while (j < i) {
							if (strcmp(image_args->colormap_name[j], p->d_name)
									== 0) {
								asprintf(&image_args->colormap_name[i],
										"%s_lookup", strdup(p->d_name));
								break;
							} else {
								image_args->colormap_name[i] = strdup(
										p->d_name);
							}
							j++;
						}
...
void colormap_init(t_image_extract *image_args) {
	...
	image_args->premapped_colormap = malloc(
			(number_of_colormaps + number_of_lookups + 1)
					* sizeof(*image_args->premapped_colormap));
	image_args->premapped_colormap[0] = NULL;
	image_args->colormap_name = malloc(
			(number_of_colormaps + number_of_lookups + 1)
					* sizeof(*image_args->colormap_name));
	image_args->colormap_name[0] = NULL;

	if (number_of_colormaps)
		load_colormaps_from_directory(colormaps_path, image_args, FALSE);
	if (number_of_lookups)
		load_colormaps_from_directory(lookup_path, image_args, TRUE);
...
void *percentage(void *args) {
	...
	while (a->info->percent < 100) {
		...
		a->info->percent = (100 / (float) a->info->total_slices_to_do)
				* a->info->slices_done;
...
int **generate_dims_start_end(t_vol *v, int sx, int ex, int sy, int ey, int sz,
		int ez) {
	int i;
	int **dim_start_end;
...
t_image_args *create_args_thread(t_tissue_stack *t, t_vol *vol,
		t_image_extract *image_general, FILE *sock) {
	...
	if (vol->size != NULL) {
		args->volume->size = malloc(
				sizeof(*args->volume->size) * args->volume->dim_nb);
		memcpy(args->volume->size, vol->size,
				sizeof(*vol->size) * args->volume->dim_nb);
	}
	if (vol->slice_size != NULL) {
		args->volume->slice_size = malloc(
				sizeof(*args->volume->slice_size) * args->volume->dim_nb);
		memcpy(args->volume->slice_size, vol->slice_size,
				sizeof(*vol->slice_size) * args->volume->dim_nb);
	}
	if (vol->dim_offset != NULL) {
		args->volume->dim_offset = malloc(
				sizeof(*args->volume->dim_offset) * args->volume->dim_nb);
		memcpy(args->volume->dim_offset, vol->dim_offset,
				sizeof(*vol->dim_offset) * args->volume->dim_nb);
	}
	...
	if (vol->dim_name != NULL) {
		args->volume->dim_name = malloc(
				sizeof(*args->volume->dim_name) * args->volume->dim_nb);
		int i = 0;
	...
	if (vol->path != NULL)
		args->volume->path = strdup(vol->path);
	...
	if (image_general->dim_start_end != NULL) {
		args->info->dim_start_end = malloc(
				sizeof(*image_general->dim_start_end) * args->volume->dim_nb);
		int i = 0;
		while (i < args->volume->dim_nb) {
			args->info->dim_start_end[i] = malloc(
					sizeof(*image_general->dim_start_end[i]) * 2);
			args->info->dim_start_end[i][0] =
					image_general->dim_start_end[i][0];
			args->info->dim_start_end[i][1] =
					image_general->dim_start_end[i][1];
			i++;
		}
	}
...
void lunch_percent_display(t_tissue_stack *t, t_vol *vol,
		t_image_extract *image_general) {
	t_image_args *args;
...
void lunch_pct_and_add_task(t_tissue_stack *t, t_vol *vol,
		t_image_extract *image_general, int fd, char **commands) {
	char *command_line;
	char *id_percent;
	...
	if (image_general->percentage && image_general->id_percent == NULL) {
		...
		t->percent_init(get_nb_blocks_percent(image_general, vol), &id_percent,
				vol->path, "0", image_general->root_path, command_line, t);
...
void image_creation_lunch(t_tissue_stack *t, t_vol *vol, int step,
		t_image_extract *image_general, FILE *sock) {
	t_image_args *args;
	unsigned int i = 0;
	unsigned int j;
	...
	if (image_general->percentage) {
		// One thread only for each volume and all its dimensions
		args = create_args_thread(t, vol, image_general, sock);
		args->general_info = t;
		i = 0;
		args->dim_start_end = malloc(
				sizeof(*args->dim_start_end) * vol->dim_nb);
		while (i < vol->dim_nb) {
			args->dim_start_end[i] = malloc(
					sizeof(*args->dim_start_end[i]) * 2);
	...
	while (i < 3) {
		if (dim_start_end[i][0] != -1 && dim_start_end[i][1] != -1) {
			...
			nb_slices = ((
					dim_start_end[i][1] == 0 ?
							vol->size[i] : dim_start_end[i][1])
					- dim_start_end[i][0]);
			while (j < nb_slices) {
				args = create_args_thread(t, vol, image_general, sock);
				args->general_info = t;
				if ((dim_start_end[i][0] + step) <= dim_start_end[i][1])
					args->dim_start_end = generate_dims_start_end_thread(vol, i,
							dim_start_end[i][0], dim_start_end[i][0] + step);
				else
					args->dim_start_end = generate_dims_start_end_thread(vol, i,
							dim_start_end[i][0], dim_start_end[i][1]);
#ifdef _NO_TILING_THREAD_
				get_all_slices_of_all_dimensions(args);
#else
				(*t->tp->add)(get_all_slices_of_all_dimensions, (void *) args,
						t->tp);
#endif
				j += step;
...
int check_range(int **d, t_vol *v) {
	...
	while (v && i < v->dim_nb) {
		if (d && d[i] && (d[i][0] != -1 || d[i][1] != -1)) {
			if (d[i][0] > v->size[i] || d[i][1] > v->size[i] || d[i][0] < 0
					|| d[i][1] < 0)
				return (1);
		}
		i++;
...
void *start(void *args) {
	...
	i = 0;
	colormap = (
			strcmp(image_args->service, "tiles") == 0 ?
					strdup(a->commands[14]) :
					(strcmp(image_args->service, "full") == 0 ?
							strdup(a->commands[11]) : strdup(a->commands[15])));
	...
	while (image_args->colormap_name[i] != NULL) {
		if (strncmp(colormap, image_args->colormap_name[i], strlen(colormap))
				== 0
				&& strlen(image_args->colormap_name[i]) == strlen(colormap))
			break;
		i++;
	}
	...
	image_args->contrast_min = (
			strcmp(image_args->service, "tiles") == 0 ?
					atoi(a->commands[15]) :
					(strcmp(image_args->service, "full") == 0 ?
							atoi(a->commands[12]) : atoi(a->commands[16])));
	image_args->contrast_max = (
			strcmp(image_args->service, "tiles") == 0 ?
					atoi(a->commands[16]) :
					(strcmp(image_args->service, "full") == 0 ?
							atoi(a->commands[13]) : atoi(a->commands[17])));
	...
	step = (strcmp(image_args->service, "tiles") == 0 ?
			atoi(a->commands[17]) :
			(strcmp(image_args->service, "full") == 0 ?
					atoi(a->commands[14]) : atoi(a->commands[18])));

	image_args->request_id = (
			strcmp(image_args->service, "tiles") == 0 ?
					strdup(a->commands[18]) :
					(strcmp(image_args->service, "full") == 0 ?
							strdup(a->commands[15]) : strdup(a->commands[19])));
	image_args->request_time = (
			strcmp(image_args->service, "tiles") == 0 ?
					strdup(a->commands[19]) :
					(strcmp(image_args->service, "full") == 0 ?
...
void free_image_extract(t_image_extract * extract) {
	...
	if (extract->dim_start_end != NULL) {
		...
		while (i < extract->dim_nb) {
			if (extract->dim_start_end[i] != NULL)
				free(extract->dim_start_end[i]);
			i++;
		}
		free(extract->dim_start_end);
	}

	if (extract->image_type != NULL)
		free(extract->image_type);
	if (extract->root_path != NULL)
		free(extract->root_path);
	if (extract->service != NULL)
		free(extract->service);
	if (extract->request_id != NULL)
		free(extract->request_id);
	if (extract->request_time != NULL)
		free(extract->request_time);
...
void free_image_args(t_image_args * args) {
	...
	if (args->dim_start_end != NULL) {
		int i = 0;
		while (i < args->volume->dim_nb) {
			if (args->dim_start_end[i] != NULL)
				free(args->dim_start_end[i]);
			i++;
		}
	...
	if (args->volume != NULL) {
		if (args->volume->size != NULL)
			free(args->volume->size);
		if (args->volume->path != NULL)
			free(args->volume->path);

		if (args->volume->dim_name != NULL) {
			int i = 0;
			while (i < args->volume->dim_nb) {
				if (args->volume->dim_name[i] != NULL)
					free(args->volume->dim_name[i]);
				i++;
			}
			free(args->volume->dim_name);
		}
		if (args->volume->starts != NULL)
			free(args->volume->starts);
		if (args->volume->steps != NULL)
			free(args->volume->steps);
		if (args->volume->dim_name_char != NULL)
			free(args->volume->dim_name_char);
		if (args->volume->dim_offset != NULL)
			free(args->volume->dim_offset);
		if (args->volume->slice_size != NULL)
			free(args->volume->slice_size);
...
inline short checkRequestTimeout(t_image_args *a, Image * img,
		ImageInfo * image_info) {
	if (a == NULL
			|| a->general_info->tile_requests->is_expired(
					a->general_info->tile_requests, a->info->request_id,
					a->info->request_time)) {
