...
CHECK_SIZE=`stat -c%s /tmp/build_mod_load`
if [ $CHECK_SIZE -ne 0 ]; then echo -e "FAILED\nERROR: Could not locate needed dcmtk package (3.6.0). Please install it and load it: 'module load graphicsmagick/1.3.18' !!!!!\n"
fi
...
module load tissuestack/2.1 2> /tmp/build_mod_load
